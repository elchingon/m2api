module M2api
  class Base
    ...
    def save
      ...
      response = M2api::Base.call :put, "#{self.class.base_path}/#{self.send(self.class.unique_identifier)}", self
      self.class.new response
    end

    def delete
      self.check_presence self.class.unique_identifier
      M2api::Base.call :delete, "#{self.class.base_path}/#{self.send(self.class.unique_identifier)}", nil
    end
    ...
    def check_presence *attributes
      M2api::Validator.check_presence self, *attributes
    end
    ...
    class << self
      ...
      def [] unique_identifier_value
        result = M2api::Base.get nil, path:"#{base_path}/#{unique_identifier_value}"
      ...
      def each_page page_size=M2api.default_page_size, &block
      ...
      def call method, path=self.base_path, params
        M2api::Base.adapter.call(method, path, params)
      end
