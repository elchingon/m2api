module M2api
  class Order < Base
    ...
    def products
      ...
      M2api::Product.filter(:sku => product_skus).all
    end

    def customer
      self.check_presence :customer_id
      M2api::Customer[self.customer_id]
    end
    ...
    class << self
      ...
      def unique_identifier
        # M2api::Logger.error "orders do not contain id on default requests, therefore they cannot be targeted on the API"
        :entity_id
      end
