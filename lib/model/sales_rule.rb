module M2api
  class SalesRule < Base
    ...
    def generate quantity:1, length:16, format:(:alpha), delimiter:"-", delimiter_at_every:4
      ...
      M2api::Coupon.generate self.rule_id, quantity:quantity, length:length, format:format, delimiter:delimiter, delimiter_at_every:delimiter_at_every
    end

    def coupons
      self.check_presence :rule_id
      M2api::Coupon.filter(:rule_id => self.rule_id).all
    end
